...
class MemoryMonitor:
    ...
    def estimate_paged_writer_block_memory(
        self,
        block_size: int,
        *,
        ...
        if self._kv_bytes_per_token_override is not None:
            total = block_size * self._kv_bytes_per_token_override
        else:
            total = self.estimate_block_memory(
                block_size,
        ...
        if (
            self._prefill_memory_profile is None
            and self._rotating_layer_specs
        ):
