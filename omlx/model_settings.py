...
class ModelSettings:
    ...
    Attributes:
        ...
        turboquant_mid_prefill: Convert dense KV once under prefill memory pressure.
    ...
    def __post_init__(self) -> None:
        ...
        # Mid-prefill conversion clears process-wide Metal buffers and must own
        # the sole engine lane. DFlash can create a fallback engine later, so
        # supporting both would require a second ownership transition.
        if (
            self.dflash_enabled
            and self.turboquant_kv_enabled
            and self.turboquant_mid_prefill
        ):
            raise ValueError(
                "turboquant_mid_prefill cannot be enabled with dflash_enabled"
            )
